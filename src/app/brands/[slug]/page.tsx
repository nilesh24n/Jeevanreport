...
export function generateStaticParams() {
  return getAllBrands().slice(0, 20).map((b) => ({ slug: b.slug }));
}
...
export default async function BrandPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const withChanges = brandProducts.filter(
    (p) => (p.packSizeChanges?.length || 0) > 0 || (p.formulaChanges?.length || 0) > 0
  );
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 sm:py-8">
      ...
      <div className="my-4">
        <h1 className="text-2xl sm:text-3xl font-bold text-slate-900">{brand.name}</h1>
        <p className="mt-2 text-sm sm:text-base text-slate-600">
          {brandProducts.length} products · Markets: {brand.countries.join(", ")} · {brand.avgTrust}% avg trust
        </p>
      </div>
